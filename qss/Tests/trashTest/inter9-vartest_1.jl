...
coeffs=NTuple{3,Float64}((-0.31695796387035857, -0.02797046795565248, -0.001))
coeffs2=NTuple{3,Float64}((-0.27695796387035854, 0.016029532044347518, 0.001))
pp=pointer(Vector{NTuple{2,Float64}}(undef, 3))
res = pointer(Vector{Float64}(undef, 2))
#@show pp
...
count=allrealrootintervalnewtonregulafalsi(coeffs,res,pp)
@show count
resTup=(unsafe_load(res,1),unsafe_load(res,2))
resfilterd=filter((x) -> x >0.0 , resTup)
display(resfilterd)
@show resTup
count=allrealrootintervalnewtonregulafalsi(coeffs2,res,pp)
@show count
resTup=(unsafe_load(res,1),unsafe_load(res,2))
resfilterd=filter((x) -> x >0.0 , resTup)
display(resfilterd)
