...
function solveProblem(prblem::Function,ft::Float64,solver::Tuple{Val{solType}, Val{V}},absTol,relTol)where {V,solType} 
    ...
    x1=pr[2]
    x2=pr[3]
    #display(plot!(x1,xlims=(0,100)));display(plot!(x2,xlims=(0,100)));readline();println("press keyboard")
    #= p1=plot!(x1,xlims=(0,100));p1=plot!(p1,x2,xlims=(0,100))
    
    savefig(p1, "plot_$(prblem).png") =#
    timenmliqss=0.0
    #= absTol=1e-5
    relTol=1e-3 =#
    solnmliqss2=QSS_Solve(odeprob,solver,dQmin=absTol,dQrel=relTol,finalTime=ft,maxErr=100*relTol)
  #  save_Sol(solnmliqss2,"";xlims=(1.8,10.0),ylims=(-0.25,0.53))
...
function mainTest()
    ...
    cfuns=[C11,C12,C13,C14,C15,C16,C17,C18,C19,C20]
   # cfuns=[C1,C2,C3,C4,C5,C6,C7,C8,C9,C10]
   # cfuns=[C9]
    #cfuns=[C1,C2,C3,C4]
...
Problemletter=['B','C','D','E','F','G','H','I','J','K','L','M','N','O','P','Q','R','S','T']#for excel not be confused with problem name
          ...
          #= XLSX.openxlsx("error_LTI_C11_C20_nmliqss2_Iter_recompute_1e-13.xlsx", mode="w") do xf
