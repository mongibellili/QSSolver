  ...
  newEx=postwalk(ex) do a  # change rhs equ to contain q[] and d[] instead of user symbols
      if a isa Expr && a.head == :ref # a is expre of type A[n]  ie a=A[n]
          ...
          if a.args[1]==stateVarName 
              a.args[1]=:q ######maybe x is better for zc...if thats the case have another array of symbols
           else
               a.args[1]=:d
           end
      end
...
function changeVarNames_to_x_d(ex::Expr,stateVarName::Symbol)
newEx=postwalk(ex) do a  # change rhs equ to contain q[] and d[] instead of user symbols
    if a isa Expr && a.head == :ref # a is expre of type A[n]  ie a=A[n]
        a.args[1]!=stateVarName && a.args[1]!=:discrete && error("symbol $(a.args[1]) undefined")
        if a.args[1]==stateVarName 
            a.args[1]=:x ######maybe x is better for zc...if thats the case have another array of symbols
         else
             a.args[1]=:d
         end
    end
...
function twoInOneSimplecase(ex)
ex=Expr(:call, :createT,ex)
...
#############################minus sign#############################################
    ...
    elseif x isa Expr && x.head == :call && x.args[1] == :- && length(x.args) == 2  #negate
            if i <= ex.args[2]
...
############################### multiply sign#######################################
  ...
  #return ex ########################################************************
  ex.args[2]=length(cachexpr_lengthtracker.args)# return the number of caches distributed...later clean only these
  # @show ex.args[2]
  # @show i
   return ex
