function nmisCycle_and_simulUpdate(::Val{M},cacheRootsi::Vector{Float64},cacheRootsj::Vector{Float64},acceptedi::Vector{Vector{Float64}},acceptedj::Vector{Vector{Float64}},aij::Float64,aji::Float64,respp::Ptr{Float64}, pp::Ptr{NTuple{2,Float64}},trackSimul::Vector{Int},::Val{2},index::Int,j::Int,dirI::Float64, x::Vector{Taylor0},q::Vector{Taylor0}, quantum::Vector{Float64},exactA::Function,d::Vector{Float64},cacheA::MVector{1,Float64},dxaux::Vector{MVector{2,Float64}},qaux::Vector{MVector{2,Float64}},tx::Vector{Float64},tq::Vector{Float64},simt::Float64,ft::Float64)where {M}  
  ...
  if iscycle
        ...
        if (abs(qi - xi) > 2.0*quani || abs(qj - xj) > 2.0*quanj) 
          h1 = sqrt(abs(1*quani/ddxi));h2 = sqrt(abs(1*quanj/ddxj));   #later add derderX =1e-12 when x2==0?
          h=min(h1,h2)
          Δ1=1.0-h*(aiijj)-h*h*(aiijj_)
          if abs(Δ1)!=0.0 
            h_2=h*h;h_3=h_2*h;h_4=h_3*h;h_5=h_4*h;h_6=h_5*h
        ...
        maxIter=10000
        while (abs(qi - xi) > 1.0*quani || abs(qj - xj) > 1.0*quanj) && (maxIter>0)
          maxIter-=1
