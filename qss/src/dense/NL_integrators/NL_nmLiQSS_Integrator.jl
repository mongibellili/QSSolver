function integrate(Al::QSSAlgorithm{:nmliqss,O},options::Options{SU,DU}, CommonqssData::CommonQSS_data{0}, liqssdata::LiQSS_data{O,M}, odep::NLODEProblem{PRTYPE,T,0,0,CS}, f::Function, jac::Function, SD::Function, exactA::Function) where {PRTYPE,CS,O,T,SU,DU,M}
  ...
  qaux = liqssdata.qaux
  dxaux = liqssdata.dxaux #= olddx=liqssdata.olddx;  =#
  ...
  for i = 1:T
    ...
    if isempty(jac(i))
      ...
      aii=prepareAii(i,q, exactA, d, cacheA,initTime)
  ...
  while simt < ft && totalSteps < maxiters
    ...
    if sch[3] == :ST_STATE
      ...
      aii=prepareAii(index,q, exactA, d, cacheA,simt)
      ...
      for j in SD(index)
       ...
       #=  cacheA[1] = 0.0
        ...
        aii,aij,aji,ajj=prepareA(index,j,q, exactA, d, cacheA,simt) 
  ...
  createSol(Val(T), Val(O), savedTimes, savedVars, "nmliqss$(O)_single$(SU)_simul$(DU)_detect$(M)", string(odep.prname), absQ, stats, ft)
end
