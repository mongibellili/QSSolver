function integrate(Al::QSSAlgorithm{:mliqss,O},options::Options{SU,DU}, CommonqssData::CommonQSS_data{0}, liqssdata::LiQSS_data{O,M}, odep::NLODEProblem{PRTYPE,T,0,0,CS}, f::Function, jac::Function, SD::Function, exactA::Function) where {PRTYPE,CS,O,T,SU,DU,M}
  ...
  for i = 1:T
    ...
    if isempty(jac(i))
      ...
      updateQ(Val(O),options, i, x, q, quantum, aii, dxaux, qaux, tx, tq, initTime + 1e-12, ft, nextStateTime) #1e-9 exactAfunc contains 1/t???
    end
  end
  for i = 1:T
  ...
  while simt < ft && totalSteps < maxiters
    ...
    if sch[3] == :ST_STATE
      ...
      integrateOldx(Val(O),x[index],olddx[index],elapsed)
      tx[index] = simt
      ...
      aii=prepareAii(index,a)
       updateQ(Val(O),options, index, x, q, quantum, aii, dxaux, qaux, tx, tq, simt, ft, nextStateTime)
      tq[index] = simt
      ...
      for j in SD(index)
       ...
       #=  cacheA[1] = 0.0
        ...
        aii,aij,aji,ajj=prepareA(index,j, a) 
       
        if j != index && aij * aji != 0.0
          integrateVarJ(j,x,tx,simt,olddx)
           if nmisCycle_and_simulUpdate(Val(O),options,Val(M),cacheRootsi, cacheRootsj, acceptedi, acceptedj, aii,aij, aji,ajj, respp, pp, trackSimul,  index, j, dirI, x, q, quantum,  dxaux, qaux, tx, tq, simt, ft)  
            ...
            for k in SD(j)  #j influences k
              if k != index && k != j
                ...
                integrateOldx(Val(O),x[k],olddx[k],elapsedx)
                tx[k] = simt
                elapsedq = simt - tq[k]
      ...
      for c in SD(index)   #index influences c       
        ...
        if elapsedx > 0 x[c].coeffs[1] = x[c](elapsedx) ;tx[c] = simt;integrateOldx(Val(O),x[c],olddx[c],elapsedx) end # 
        elapsedq = simt - tq[c]
        ...
        updateOtherApprox(c,index,x,q,a,qaux,olddx,simt)
      end#end for SD
      updateLinearApprox(index,x,q,a,qaux,olddx,simt)
    ##################################input########################################
