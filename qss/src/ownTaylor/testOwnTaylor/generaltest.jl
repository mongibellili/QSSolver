...
function clearCache(cache::Taylor0{T}) where {T<:Number}
      cache.coeffs.=0.0
end
function closeEnough(sum1::Taylor1{Float64},sum2::Taylor0{Float64}) 
    isclose=true
...
function closeEnough(sum1::Float64,sum2::Taylor0{Float64}) 
        ...
        for (term11,term01) in((:a1,:a0),(:e,:e))
            for (term12,term02) in((:a1,:a0),(:b1,:b0))
                for (term13,term03) in((:c1,:c0),(:f,:f))
                    ...
                    @eval begin
                    clearCache(cache[1])
                    @assert closeEnough($op3($op1($op2($term12,$term13),$term11),$term14), @changeAST ($op3($op1($op2($term02,$term03),$term01),$term04),1))
                    end 
                    end
                end
            end
...
#stressTest(a1,b1,c1,d1,a0,b0,c0,d0,cache,e,f)
    ...
    for op3 in (:abs,:exp,:cos,:sin)
        for (term11,term01) in((:a1,:a0),(:e,:e))
            for (term12,term02) in((:a1,:a0),(:b1,:b0))
                    @eval begin
                    clearCache(cache[1])
                    @assert closeEnough($op3($op1($op2($term12))), @changeAST ($op3($op1($op2($term02))),1))
                    @assert closeEnough($op3($op1($op2($term11))), @changeAST ($op3($op1($op2($term01))),1))
                    end                               
            end
        end
    end
