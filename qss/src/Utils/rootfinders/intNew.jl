...
@inline function smallest(args::NTuple{N,Float64})::Float64 where {N}
	@inbounds m = args[1]
	for i in 2:N
		@inbounds arg = args[i]
		m = arg < m ? arg : m
...
@inline function horner(x::Float64, coeffs::NTuple{N,Float64})::Float64 where {N}
	@inbounds v = coeffs[1]
	for i in 2:N
...
@inline function horner2(x::Float64, y::Float64, coeffs::NTuple{N,Float64})::Tuple{Float64, Float64} where {N}
	@inbounds v = coeffs[1]
	w = v
	for i in 2:N
		@inbounds coeff = coeffs[i]
...
@inline function hornerd(x::Float64, coeffs::NTuple{N,Float64})::Tuple{Float64, Float64} where {N}
	@inbounds v = coeffs[1]
	d = 0.0
	for i in 2:N
		@inbounds coeff = coeffs[i]
...
@inline function posintervalhorner(low::Float64, high::Float64, coeffs::NTuple{N,Float64})::Tuple{Float64, Float64} where {N}
...
function smallestpositiverootintervalnewtonregulafalsi(coeffs::NTuple{N,Float64}, doms::Ptr{NTuple{2,F}})::Float64 where {N, F <: AbstractFloat}
	if N == 1
		return typemax(F)
	elseif N == 2
...
function posIntNewton(coeffs::NTuple{N,Float64}, doms::Ptr{NTuple{2,Float64}})::Float64 where {N}
	pr=()
    @inbounds _coeff1 = inv(coeffs[1])
    poly = ntuple(N) do i
        @inbounds _coeff1 * coeffs[i]
...
#coeffs=NTuple{4,Float64}((-1.33,2.0,1.0,6.0))
#coeffs=NTuple{4,Float64}((-1.33,2.0,1.0,6.0))
#coeffs=NTuple{3,Float64}((14.691504647354595,-747452.6968034876,1.0e-6))
...
#@show smallestpositiverootintervalnewtonregulafalsi(coeffs,pp)# 93.687 ns (0 allocations: 0 bytes)
...
#@btime gensmallestpositiverootintervalnewton(1.0, -2.0, -1.0)#64.912 ns (1 allocation: 32 bytes)
#@show gensmallestpositiverootintervalnewton(1.0, -2.0, -1.0)#2.414213562506035#@show quadraticsmallestpositiveroot(1.0, -2.0, -1.0)#1.649 ns (0 allocations: 0 bytes)
@show smallestpositiverootintervalnewtonregulafalsi(coeffs2,pp) =#