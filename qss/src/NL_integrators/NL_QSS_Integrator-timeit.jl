using TimerOutputs
function QSS_integrate(::Val{O}, s::QSS_data, odep::NLODEProblem) where {O}
#*********************************settings*****************************************
reset_timer!()
...
#for k = 1:order # compute initial derivatives for x and q (similar to a recursive way )
  ...
  for i = 1:states
    clearCache(taylorOpsCache,cacheSize)
    @timeit "f" f(i,q,d, t ,taylorOpsCache)
...
for i = 1:states
  ...
  @timeit "computenexttime" computeNextTime(Val(O), i, initTime, nextStateTime, x, quantum)
  clearCache(taylorOpsCache,cacheSize)
  ...
  @timeit "computenextinputtime" computeNextInputTime(Val(O), i, initTime, 0.1,taylorOpsCache[1] , nextInputTime, x,  quantum)

end
#println("before zc who is resizing?")
for i=1:numberZC
  clearCache(taylorOpsCache,cacheSize)
  @timeit "zcf" output=zcf[i](x,d,t,taylorOpsCache).coeffs[1] #test this evaluation
  oldsignValue[i,2]=output #value
  oldsignValue[i,1]=sign(output) #sign modify 
  @timeit "computenextevent" computeNextEventTime(i,output,oldsignValue,initTime,  nextEventTime, quantum,printCounter)
end
...
while simt < ft #&& printcount < 3
  #printcount += 1
  @timeit "update scheduler" sch = updateScheduler(nextStateTime,nextEventTime, nextInputTime)
  simt = sch[2]
  index = sch[1]
  t[0]=simt
 # println("index= ",index)
  ...
  #=   if printCounter[2] > 0 && 1.4386 < simt < 1.4388
    ...
    @timeit "integratestate" integrateState(Val(O),x[index],integratorCache,elapsed)
    ...
    for i = 1:length(SD[index])
      ...
      if j != 0           
       ...
       # @timeit "state-f"  
        ...
        f(j,q,d,t,taylorOpsCache)
        @timeit "computederiv" computeDerivative2(Val(O), x[j], taylorOpsCache[1])
        @timeit "recomputenext" reComputeNextTime(Val(O), j, simt, nextStateTime, x, q, quantum)
        #=  if printCounter[2] > 0
          printCounter[2]-=1
         println("x[$j]= ",x[j])
         end =#
