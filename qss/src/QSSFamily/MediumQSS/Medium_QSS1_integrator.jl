...
function QSS_integrate(::Val{1}, s::MediumQSS_data, settings::ProblemSetting, prob::MediumProblem)
  #reset_timer!()
  #*********************************settings*****************************************
 ...
 #=  f(q::MVector{R,Float64} ,tq::MVector{T,Float64} ,order::Int) where {R,T}=q[2] 
  ...
  for i = 1:states
    computeDerivative(i, solver, x, q, tx, tq,jacobian)
  end
  for i = 1:states
    ...
    while t < ft
      ...
      for i = 1:length(dep[index])
        if dep[index][i] != 0
         ...
         computeDerivative(j, solver, x, q, tx, tq,jacobian)
