...
for i = 1:length(odep.eventEqus)# later change to numberZC
 ...
 # println("odep.eventEqus[i].args[2])= ", odep.eventEqus[i])
  push!(eventf, @RuntimeGeneratedFunction(odep.eventEqus[i].args[2])) #args[2] not needed because the way the expr was constructed in the qssnlproblem is different
end
...
#=   x = Taylor0{Float64}[]
  q = Taylor0{Float64}[] =#
  t = Taylor0(order)
  cacheT2=Taylor0(order) #for integratestate only
  cacheT2[1]=0.0# completely empty

  cacheT=[Taylor0([0.0,0.0,0.0],2),Taylor0([0.0,0.0,0.0],2),Taylor0([0.0,0.0,0.0],2),Taylor0([0.0,0.0,0.0],2),Taylor0([0.0,0.0,0.0],2),Taylor0([0.0,0.0,0.0],2),Taylor0([0.0,0.0,0.0],2),Taylor0([0.0,0.0,0.0],2),Taylor0([0.0,0.0,0.0],2),Taylor0([0.0,0.0,0.0],2),Taylor0([0.0,0.0,0.0],2),Taylor0([0.0,0.0,0.0],2)]
  #t=Taylor0([1e-120,1,0],order-1) # this is bad cuz 1/t explodes
...
#=   if Debug
  ...
  for k = 1:order # compute initial derivatives for x and q (similar to a recursive way )
    ...
    for i = 1:states
      ...
      clearCache(cacheT)
      x[i].coeffs[k+1] = ((differentiate(f[i](q,d, t + initTime,cacheT), k - 1)).coeffs[1]) / factorial(k) # /fact cuz i will store der/fac like the convention...to extract the derivatives (at endof sim) multiply by fac  derderx=coef[3]*fac(2)
    end
  end
...
#println("f= ",f)
  for i = 1:states
    ...
    #cacheT.coeffs.=0.0 #clear cache
    clearCache(cacheT)
    computeNextInputTime(Val(2), i, initTime, 0.0,f[i](q,d,initTime + t,cacheT), nextInputTime, x,  quantum)
  end
  #@show cacheT
  for i=1:numberZC
    #println("event i= ", i)
    ...
    output=zcf[i](q,d,initTime + t,cacheT).coeffs[1] #test this evaluation
 ...
 # counter=MVector{1,Int}(6) # delete later ...used for limited println 
  for i=1:numberZC
    clearCache(cacheT)
    computeNextEventTime(i,zcf[i](q,d,initTime + t,cacheT),oldsignValue,initTime,  nextEventTime, quantum)
  end
...
#=     if Debug
  ...
  #=    for k = 1:order #evaluate x at new time, and derivatives...remember coeffs store the over facto also              
   ...
   #cacheT.coeffs.=0.0 #clear cache
   #println("inside state cache before integratestate= ",cacheT) 
   clearCache(cacheT)
   integrateState(Val(2),x[index],cacheT2,elapsed)
   #println("inside state cache after integratestate= ",cacheT) 
     ...
     # println("at time $simt state change  index= ",index)
      ...
      for i = 1:length(SD[index])
       ...
       #    println("dependency loop $j= ")              
         ...
         # cacheT.coeffs.=0.0 #clear cache
#println("cache before integrate!= ",cacheT) 
          clearCache(cacheT)
          integrate!(x[j],f[j](q,d,t,cacheT),x[j].coeffs[1])

        #  println("cache after integrate!= ",cacheT) 
      ...
      for i = 1:length(SZ[index])
        j = SZ[index][i] # this line before the next line or vice versa gave the same bench results
         if j != 0             
          ...
          clearCache(cacheT)
          computeNextEventTime(j,zcf[j](q,d,simt + t,cacheT),oldsignValue,simt,  nextEventTime, quantum)
    ...
    elseif sch[3] == :ST_INPUT  # time of change has come to a state var that does not depend on anything...no one will give you a chance to change but yourself  
     ...
     # println("from integrate: input change after the event index= ",index)
    # cacheT.coeffs.=0.0 #clear cache
    # println("inside input cache before computenextinputtime= ",cacheT) 
    clearCache(cacheT)
      computeNextInputTime(Val(2), index, simt, elapsed,  f[index](q,d,simt + t,cacheT), nextInputTime, x,  quantum)
     # computeDerivative(Val(2), index,x, f[index](q,d,t),cacheT,simt)
      for i = 1:length(SD[index])
       ...
       ####   computeDerivative(Val(2), j,tq,x,q,d,t,f[j],cacheT,simt)
        #  x[j]=integrate(f[j](q,d,t),x[j].coeffs[1])
       # cacheT.coeffs.=0.0 #clear cache
        #println("inside input cache before integrate!= ",cacheT) 
        clearCache(cacheT)
          integrate!(x[j],f[j](q,d,t,cacheT),x[j].coeffs[1])
         # println("inside input cache after integrate!= ",cacheT) 
          reComputeNextTime(Val(2), j, simt, nextStateTime, x, q, quantum)
        end#end if j!=0
      end#end for
   ...
   #################################################################event########################################
    ...
    #=   println("zcf[index](q,d,simt + t)=   ",zcf[index](q,d,simt + t))
     ...
     clearCache(cacheT)
     if (zcf[index](q,d,simt + t,cacheT).coeffs[1])>0       # sign is not needed here
      ...
      clearCache(cacheT)
      eventf[modifiedIndex](q,d,simt + t,cacheT)
      ...
      for i = 1:length(HD[modifiedIndex]) # care about dependency to this event only
        ...
        #      println("inside integrate:event --the state var affected by event is x$j")
              ...
              #    println("before derivative event---------------------------------- d= ",d)
                ...
                #  computeDerivative(Val(2), j,tq,x,q,d,t, f[j],cacheT,simt)
                #  x[j]=integrate(f[j](q,d,t),x[j].coeffs[1])
               # println("inside event cache before integrate!= ",cacheT) 
              #  cacheT.coeffs.=0.0 #clear cache
              clearCache(cacheT)
                  integrate!(x[j],f[j](q,d,t,cacheT),x[j].coeffs[1])
                 # println("inside event cache after integrate!= ",cacheT) 
                  reComputeNextTime(Val(2), j, simt, nextStateTime, x, q, quantum)
