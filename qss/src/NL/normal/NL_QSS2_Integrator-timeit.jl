...
while simt < ft #&& count < 4
  ...
  @timeit "scheduler" sch = updateScheduler(nextStateTime,nextEventTime, nextInputTime)
  simt = sch[2]
  index = sch[1]
  ##########################################state########################################
  if sch[3] == :ST_STATE
    elapsed = simt - tx[index]
    @timeit "state-integrate state" integrateState(Val(2),x[index],integratorCache,elapsed)
    ...
    @timeit "state-computeNextTime" computeNextTime(Val(2), index, simt, nextStateTime, x, quantum) #
    for i = 1:length(SD[index])
      ...
      if j != 0           
       ...
       # @timeit "state-clearCache" #clearCache(taylorOpsCache,cacheSize)
        ...
        # @timeit "state-integrate!" integrate!(x[j],f[j](q,d,t,taylorOpsCache),x[j].coeffs[1])
         @timeit "state-icomputeder" computeDerivative2(Val(2), x[j],f[j](q,d,t,taylorOpsCache))
         @timeit "state-reComputeNextTime" reComputeNextTime(Val(2), j, simt, nextStateTime, x, q, quantum)
      end#end if j!=0
    end#end for SD
  ...
  elseif sch[3] == :ST_INPUT  # time of change has come to a state var that does not depend on anything...no one will give you a chance to change but yourself  
    elapsed = simt - tx[index]    
    #clearCache(taylorOpsCache,cacheSize)
    @timeit "Input-computeNextInputTime" computeNextInputTime(Val(2), index, simt, elapsed,  f[index](q,d,simt + t,taylorOpsCache), nextInputTime, x,  quantum)
    for i = 1:length(SD[index])
      j = SD[index][i] 
      if j != 0             
        elapsed = simt - tx[j]
  ...
  @timeit "saving" if simt > savetime
      count += 1
