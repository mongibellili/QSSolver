...
for i = 1:states# x and q get init conditions
  ...
  push!(derx, Taylor0(zeros(order+1), order))
...
for k = 1:order # compute initial derivatives for x and q (similar to a recursive way )
  ...
  for i = 1:states
    ...
    f(i,derx,q,d, t + initTime,taylorOpsCache)
...
#=   for j = 1:interpolationOrder
  ...
  clearCache(taylorOpsCache,cacheSize)
  f(i,derx,q,d,initTime + t,taylorOpsCache)
...
while simt < ft #&& count < 4
  ...
  elseif sch[3] == :ST_INPUT  # time of change has come to a state var that does not depend on anything...no one will give you a chance to change but yourself  
    elapsed = simt - tx[index]    
    clearCache(taylorOpsCache,cacheSize)
    f(index,derx,q,d,simt + t,taylorOpsCache)
...
#= function f(j::Int,derx::Vector{Taylor0{Float64}},q::Vector{Taylor0{Float64}},d::Vector{Float64}, t::Taylor0{Float64},cache::Vector{Taylor0{Float64}})
  if j == 1
    ...
    #= none:1 =#
    derx[2] = subT(negateT(q[1], cache[2]), q[2], cache[1])
    #= none:1 =#
    return nothing
end
end =#