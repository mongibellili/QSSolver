   ...
   function qss_Integrate(initCond::Vector{Float64},f::SVector{N, Function},order::Int)where{N}
       ...
       Debug=false
        ...
        for i = 1:states
            ...
            quantum[i] = relQ * abs(x[i].coeffs[2]) #derx=coef[2]*fac(1), derderx=coef[3]*fac(2)
        ...
        while simt < ft #&& count < 10
           ...
           elapsed = simt - tx[index]
             #shift taylor
            x[index].coeffs[1]=x[index](t+elapsed).coeffs[1] #evaluate x at new time
            #for higher orders add x.coeffs[]=.......
            if Debug println("loop $count :x after evaluatex = ",x) end
            quantum[index] = relQ * abs(x[index].coeffs[2]) #derx=coef[2]*fac(1), derderx=coef[3]*fac(2)            
