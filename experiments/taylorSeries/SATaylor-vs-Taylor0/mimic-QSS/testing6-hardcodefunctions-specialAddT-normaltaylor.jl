...
function addT(a::Taylor0{T}, b::Taylor0{T},c::Float64,cache::Taylor0{T}) where {T<:Number}
...
@inline function f2(x::Vector{Taylor0},j::Int,q::Vector{Taylor0},cache::Taylor0)
    if j==1
        temp=addT(q[1],q[2],1.0,cache)#in real qss this becomes temp=$(twoinone(ex.args...))
        x[1][0]=temp[0]
        x[1][1]=temp[1]
        x[1][2]=temp[2]
