...
function addT(a::Taylor1{T}, b::Taylor1{T},c::T,cache::Taylor1{T}) where {T<:Number}
  ...
  function addT(a::Taylor1{T}, b::T,c::T,cache::Taylor1{T}) where {T<:Number}
    ...
    function addT( a::T,b::T,c::T,cache::Taylor1{T}) where {T<:Number}
      #cache.coeffs.=0.0 #a is number ok to empty
      #i think emptying the cache once before the equation is cheaper than emptying it 
                      #for every op
      cache[0]=a+b+c    
       return cache
     end
...
function muladdT(a::P,b::Q,c::R,cache1::Taylor1{T},cache2::Taylor1{T}) where {P,Q,R <:Union{Taylor1,Number},T<:Number}
...
function fastArith(a::Vector{Taylor1{Float64}},b::Vector{Taylor1{Float64}},c::Vector{Taylor1{Float64}},d::Vector{Taylor1{Float64}},cache::Vector{Taylor1{Float64}})
  #@changeAST 1.2-2.1*a[1]-2.04+b[2]*3.2-c[1]-d[1]+1.12*3.2-d[2],12
 #sum=@changeAST 2.0*3.0+a[1]*b[1],6
 #@changeAST 1.2-2.1*a[1]-2.04+b[2]*3.2-c[1]-d[1]+1.12*3.2-d[2]+1.2-2.1*a[1]-2.04,12
 @changeAST 5.0,12
end
...
function boundaryTestMix(a::Vector{Taylor1{Float64}},b::Vector{Taylor1{Float64}},c::Vector{Taylor1{Float64}},d::Vector{Taylor1{Float64}},cache::Vector{Taylor1{Float64}})
  #@assert 0.0-a[1]+a[2]*0.0==@changeAST 0.0-a[1]+a[2]*0.0,12
 ...
 # @assert closeEnough(1.2-2.1*a[1]-2.04+b[2]*3.2-c[1]-d[1]+1.12*3.2-d[2]+1.2-2.1*a[1]-2.04,@changeAST 1.2-2.1*a[1]-2.04+b[2]*3.2-c[1]-d[1]+1.12*3.2-d[2]+1.2-2.1*a[1]-2.04,12)
end
#boundaryTestMix(a,b,c,d,cache)
function benchmarkboundaryMix(a::Vector{Taylor1{Float64}},b::Vector{Taylor1{Float64}},c::Vector{Taylor1{Float64}},d::Vector{Taylor1{Float64}},cache::Vector{Taylor1{Float64}})
 # @changeAST 1.2-2.1*a[1]-2.04+b[2]*3.2-c[1]-d[1]+1.12*3.2-d[2],12 #102.636 ns (0 allocations: 0 bytes)
 # 1.2-2.1*a[1]-2.04+b[2]*3.2-c[1]-d[1]+1.12*3.2-d[2]  #405.135 ns (9 allocations: 720 bytes)
 #@changeAST 1.2-2.1*a[1]-2.04+b[2]*3.2-c[1]-d[1]+1.12*3.2-d[2]+1.2-2.1*a[1]-2.04,12 #168.464 ns (1 allocation: 80 bytes) out of caches 
end
#@btime benchmarkboundaryMix(a,b,c,d,cache)
            ...
            for term4 in(1.07,d[1]) 
              for term5 in(0.7,a[2])
                  ...
                  # @show cache
                    ...
                    clearCache(cache)
                    @assert closeEnough(term1,term2,term3,term4,term5,cache)
                  end
                end
             end
          end
