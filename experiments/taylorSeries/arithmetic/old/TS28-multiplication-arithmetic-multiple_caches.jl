...
function mulT(a::Taylor1{T}, b::Taylor1{T},cache1::Taylor1{T},cache2::Taylor1{T}) where {T<:Number}
 for k in eachindex(a)
    #println("inloop a= ",a)
   # 
      @inbounds cache2[k] = a[0] * b[k]# if we use 1 cache, cache[1] (for example) gets changed !!but we need the old value below a[1]
      @inbounds for i = 1:k
        cache2[k] += a[i] * b[k-i]  #this is the reason mul needs two caches: a is cache1
      end
