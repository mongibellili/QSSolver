...
(subadd)(a, b, c)=(+)((-)(a,b),c)# this should not exist cuz addsub has 3 args + cache[1]
  ...
  function addTfoldl(op, res, bs...)
    ...
    function mulT(a, b, c, xs...)
     ...
     # println( @code_lowered mulT(mulT( mulT(a,b,xs[end]) , c ,xs[end] ),xs[1],xs[end] ))
     # println( @code_warntype mulT(mulT( mulT(a,b,xs[end]) , c ,xs[end] ),xs[1],xs[end] ))
     println( @code_typed (mulT(a,b,xs[end]) )  )
      end
...
function mulT(a::Taylor1{T}, b::T,cache::Vector{Taylor1{T}}) where {T<:Number}
 ...
 #=  for i=0:length(a)-1 " this allocates even more
    cache[1][i]=a[i]*b
  end =#
  fill!(cache[2].coeffs, b)
  
  @__dot__ cache[1].coeffs = a.coeffs * cache[2].coeffs 
  return cache[1]
end
...
sum=@changeAST 1.0*2.3*2.5*1.0,cache
