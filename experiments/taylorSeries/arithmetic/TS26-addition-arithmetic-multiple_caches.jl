...
function boundaryTestAdd(a::Vector{Taylor1{Float64}},b::Vector{Taylor1{Float64}},c::Vector{Taylor1{Float64}},d::Vector{Taylor1{Float64}},cache::Vector{Taylor1{Float64}})
...
#boundaryTestAdd(a,b,c,d,cache)
function benchmarkboundaryAdd(a::Vector{Taylor1{Float64}},b::Vector{Taylor1{Float64}},c::Vector{Taylor1{Float64}},d::Vector{Taylor1{Float64}},cache::Vector{Taylor1{Float64}})
 
  #@changeAST 1.2+2.1+a[1]+2.04+b[2]+3.2+c[1]+d[1],12 #60.010 ns (0 allocations: 0 bytes) #last fast mode 8 args + 1 for symbol= 9 (see macro)
  #@changeAST 1.2+2.1+a[1]+2.04+b[2]+3.2+c[1]+d[1]+1.2,12 #295.295 ns (7 allocations: 560 bytes) exited fast mode: can -0.0 trick to stay in fast mode
 # 1.2+2.1+a[1]+2.04+b[2]+3.2+c[1]+d[1]+1.2#299.872 ns (7 allocations: 560 bytes)
end
#@btime benchmarkboundaryAdd(a,b,c,d,cache)
...
function stressTestAdd(a::Vector{Taylor1{Float64}},b::Vector{Taylor1{Float64}},c::Vector{Taylor1{Float64}},d::Vector{Taylor1{Float64}},cache::Vector{Taylor1{Float64}})
 #=  #test 2 vars
        ...
        for term3 in(2.07,c[1]) 
          if term1==1.2 && term2==1.33 && term3==2.07
           ...
           # @show cache
            ...
            @assert closeEnough(tterm1+term2+term3,@changeAST term1+term2+term3,12)
