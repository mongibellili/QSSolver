...
(subadd)(a, b, c)=(+)((-)(a,b),c)# this should not exist cuz addsub has 3 args + cache[1]
  ...
  function twoInOne2(ex)
    ...
    elseif x isa Expr && x.head == :call && (x.args[1]==:-  ) && length(x.args)==2  #negation
      ...
      #push!(x.args,ex.args[1].args[2])# use this when exprs used for testing
    elseif x isa Expr && x.head == :call && (x.args[1]==:+  ) && (3<=length(x.args)<=9) # later investigate alloc after 9
      push!(x.args,ex.args[2])  
      x.args[1]=:addT
...
#all some these functions can be written in terms of the smaller function (subT,addT) without cost (I think). i will test later.
...
return cache[1]
   ...
   function subsub(a::Taylor1{T}, b::T,c::T,cache::Vector{Taylor1{T}}) where {T<:Number}
    cache[1].coeffs.=a.coeffs  # if first op then needed...else cache is already  a
  ...
  function subT(a::Taylor1{T}, b::T,cache::Taylor1{T}) where {T<:Number}
   ...
   function subT(a::T, b::Taylor1{T},cache::Taylor1{T}) where {T<:Number}
    #cache[1].coeffs.=-b.coeffs  #not tested
    #cache[1].coeffs.=0.0
    @__dot__ cache.coeffs = (-)(b.coeffs) # a is number ok to modify cache[1]
   # @__dot__ cache[1].coeffs=(-)(cache[1].coeffs, b.coeffs)
   cache[0]=cache[0]+a    
    #@__dot__ cache[1].coeffs = (-)(a.coeffs, b.coeffs)
     return cache
...
function mulT(a::Taylor1{T}, b::T,cache::Vector{Taylor1{T}}) where {T<:Number}
  fill!(cache[2].coeffs, b)
  @__dot__ cache[1].coeffs = a.coeffs * cache[2].coeffs  ##fixed broadcast dimension mismatch
 # @__dot__ cache[1].coeffs = a.coeffs * b
  #= for i=0:length(a)-1
    cache[1][i]=a[i]*b
  end =#
  return cache[1]
...
function mulT(a::T, b::T,cache::Vector{Taylor1{T}}) where {T<:Number}
 ...
 #=  cache[1].coeffs[1]=0.0
  cache[1].coeffs[2]=0.0
  cache[1].coeffs[3]=0.0 =#  #manually or removed no alloc
  #= for i =1:3           #alloc
    cache[1].coeffs[i]=0.0
  end =#
   #i think emptying the cashe once before the equation is cheaper than emptying it 
                      #for every op: tested : yes outside is free
...
function fastArith(a::Vector{Taylor1{Float64}},b::Vector{Taylor1{Float64}},c::Vector{Taylor1{Float64}},d::Vector{Taylor1{Float64}},cache::Taylor1{Float64})
...
# sum=@changeAST a[1]*3.2*1.3*c[1],cache #i=4        70.905 ns (0 allocations: 0 bytes)
 ...
 #= sum1=@changeAST 2.0+a[1]+2.0+a[2],cache
 @show sum1
 cache[1].coeffs.=0.0
 cache[2].coeffs.=0.0 =#
#sum2=@changeAST 2.0*2.0*a[2]+a[1]*2.0*b[1]*c[2]-3.2-c[1]+b[2]*2.3,cache  
 #sum=@changeAST 3.2+1.3+1.2+a[1],cache
#sum2=@changeAST 2.0*2.0*a[2]+a[1]*2.0*b[1],cache
#sum=@changeAST 2.2*3.2*5.4*d[1]*2.0*c[1]*2.0,cache
sum1=@changeAST 2.3+3.2+2.1+a[2],cache
 #sum= @changeAST a[1]+3.2+a[2]+2.1,cache
 # sum=@changeAST 2.2-1.1-3.0,cache[1]
 #sum=@changeAST 3.2+1.3+1.2+a[1],cache
 # return sum
end
function normalArith(a::Vector{Taylor1{Float64}},b::Vector{Taylor1{Float64}},c::Vector{Taylor1{Float64}},d::Vector{Taylor1{Float64}},cache::Taylor1{Float64})
...
@time normalArith(a,b,c,d,cache) 
@time fastArith(a,b,c,d,cache)
