...
function afoldl(op, a, bs...)
 ...
 #=  macro changeAST(ex,cache)
    Base.remove_linenums!(ex)
    # dump(cache; maxdepth=8)
    twoInOne(ex)
   esc(twoInOne2(ex,cache))# adds cache to each call op
  end =#
  macro changeAST(ex)
    Base.remove_linenums!(ex)
    # dump(ex; maxdepth=8)
...
#= function fastArith2(a::Vector{Taylor1{Float64}},b::Vector{Taylor1{Float64}},c::Vector{Taylor1{Float64}},d::Vector{Taylor1{Float64}},cacheT::Taylor1{Float64})
    #sum=@changeAST a[1]-b[1]-c[1]-d[1]+a[2]-b[2],cacheT
    sum=@changeAST 3.0+a[1]-b[1],cacheT
    return sum
end =#
function fastArith(a::Vector{Taylor1{Float64}},b::Vector{Taylor1{Float64}},c::Vector{Taylor1{Float64}},d::Vector{Taylor1{Float64}},cacheT::Taylor1{Float64})
 # sum=@changeAST 1.1+b[1]+ 2.1,cacheT#-d[1]+a[2]-b[2],cacheT
 sum=@changeAST -(c[1]-b[1]),cacheT
 #sum=@changeAST a[1]-b[1]+c[1]-d[1]+a[2]-b[2]+c[2],cacheT
...
#@show  fastArith2(a,b,c,d,cacheT)
...
#for op in (:+, :-)
function fastTaylor(a::Vector{Taylor1{Float64}},b::Vector{Taylor1{Float64}},c::Vector{Taylor1{Float64}}, cacheTT::Taylor1{Float64})
#= for term1 in(1.0)
  for term2 in(1.0)
    for term3 in(1.0) =#
     
        #no constant
    #=     sum=@changeAST a[1] + b[1] + c[1],cacheT# addT
        sum=@changeAST a[1] + b[1] - c[1],cacheT#addsub     
        sum=@changeAST a[1] - b[1] + c[1],cacheT#subadd
        sum=@changeAST a[1] - b[1] - c[1],cacheT#subsub
        #addT and one constant
        sum=@changeAST a[1] + b[1] + 1.1,cacheT
        sum=@changeAST a[1] + 1.1 + c[1],cacheT
        sum=@changeAST 1.1 + b[1] + c[1],cacheT
      #  @show cacheT
        #addsub and one constant
        sum=@changeAST a[1] + b[1] - 1.1,cacheT
        sum=@changeAST a[1] + 1.1 - c[1],cacheT
        sum=@changeAST 1.1 + b[1] - c[1],cacheT
        
        #subadd and one constant
        sum=@changeAST a[1] - b[1] + 1.1,cacheT
        sum=@changeAST a[1] - 1.1 + c[1],cacheT
        sum=@changeAST 1.1 - b[1] + c[1],cacheT
        
        #subsub and one constant
        sum=@changeAST a[1] - b[1] - 1.1,cacheT
        sum=@changeAST a[1] - 1.1 - c[1],cacheT
        sum=@changeAST 1.1 - b[1] - c[1],cacheT

         #addT and two constants
        sum=@changeAST a[1] + 1.1 + 2.2,cacheT
        sum=@changeAST 1.1 + b[1] + 2.2,cacheT
        sum=@changeAST 1.1 + 2.2 + c[1],cacheT

          #addsub and two constants
          sum=@changeAST a[1] + 1.1 - 1.1,cacheT
          sum=@changeAST 1.1 + b[1] - 2.2,cacheT
          sum=@changeAST 1.1 + 2.2 - c[1],cacheT

          #subadd and two constants
          sum=@changeAST a[1] - 1.1 + 1.1,cacheT
          sum=@changeAST 1.1 - b[1] + 2.2,cacheT
          sum=@changeAST 1.1 - 2.2 + c[1],cacheT

          #subsub and two constants
          sum=@changeAST a[1] - 1.1 - 1.1,cacheT
          sum=@changeAST 1.1 - b[1] - 2.2,cacheT
          sum=@changeAST 1.1 - 2.2 - c[1],cacheT

          #3 constants
          sum=@changeAST 1.1 + 2.2+ 3.3,cacheT
          sum=@changeAST 1.1 + 2.2 - 3.3,cacheT
          sum=@changeAST 1.1 - 2.2 + 3.3,cacheT
          sum=@changeAST 1.1 - 2.2 - 3.3,cacheT =#

          sum=@changeAST 1.1 - b[1] - 2.2,cacheTT
      
  #=   end
  end
end =#
end
#@btime fastTaylor(a,b,c,cache)
#= 
function compareNormalTaylor(a::Vector{Taylor1{Float64}},b::Vector{Taylor1{Float64}},c::Vector{Taylor1{Float64}})
  #= for term1 in(1.0)
    for term2 in(1.0)
      for term3 in(1.0) =#
       
          #no constant
          sum= a[1] + b[1] + c[1]# addT
          sum= a[1] + b[1] - c[1]#addsub
          sum= a[1] - b[1] + c[1]#subadd
          sum= a[1] - b[1] - c[1]#subsub
...
#= 
function normalTaylor(T::Taylor1{Float64})
  for term1 in(1.0,T)
    for term2 in(1.0,T)
      for term3 in(1.0,T)   
        #@eval begin
  ...
  #@btime fastTaylor(a,b,c,cacheT)
  #@btime normalTaylor(r)
  #@btime compareNormalTaylor(a,b,c)

  function showFastTaylor(T::Taylor1{Float64},cacheT::Taylor1{Float64})
    for term1 in(1.0,T)
      for term2 in(1.0,T)
        for term3 in(1.0,T)   
          #@eval begin
            sum=@changeAST term1 + term2 + term3,cacheT
           # @show term1 + term2 + term3
            @assert sum==term1 + term2 + term3
            sum=@changeAST term1 + term2 - term3,cacheT
           # @show term1 + term2 - term3
            @assert sum==term1 + term2 - term3
            sum=@changeAST term1 - term2 + term3,cacheT
