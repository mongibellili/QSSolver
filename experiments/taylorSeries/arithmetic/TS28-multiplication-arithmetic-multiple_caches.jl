...
function mulT(a::Taylor1{T}, b::T,cache1::Taylor1{T}) where {T<:Number}
...
mulT(a::T,b::Taylor1{T}, cache1::Taylor1{T}) where {T<:Number} = mulT(b , a,cache1)

function mulT(a::Taylor1{T}, b::Taylor1{T},cache1::Taylor1{T}) where {T<:Number}
...
function mulT(a::T, b::T,cache1::Taylor1{T}) where {T<:Number}
...
function boundaryTestMul(a::Vector{Taylor1{Float64}},b::Vector{Taylor1{Float64}},c::Vector{Taylor1{Float64}},d::Vector{Taylor1{Float64}},cache::Vector{Taylor1{Float64}})
...
#boundaryTestMul(a,b,c,d,cache)
function benchmarkboundaryMul(a::Vector{Taylor1{Float64}},b::Vector{Taylor1{Float64}},c::Vector{Taylor1{Float64}},d::Vector{Taylor1{Float64}},cache::Vector{Taylor1{Float64}})
  #@changeAST 1.2*2.1*a[1]*2.04*b[2]*3.2,12 #80.858 ns (0 allocations: 0 bytes) # in the macro 6 args + 1 arg for symbol exit fast mode and goback to norma
  #@changeAST 1.2*2.1*a[1]*2.04*b[2]*3.2*c[1],12 #239.397 ns (5 allocations: 400 bytes)  # exixted fast: we can divide by 1 (trick) to stay in the fast mode
  #1.2*2.1*a[1]*2.04*b[2]*3.2*c[1]  #norma mode 235.113 ns (5 allocations: 400 bytes) 
  #@changeAST 1.2*2.1*a[1]*2.04*b[2]*3.2*c[1]*d[1],12 #300.635 ns (6 allocations: 480 bytes)
  #@changeAST 1.2*2.1*a[1]*2.04*b[2]*3.2*c[1]*d[1]*1.12,12  #342.012 ns (7 allocations: 560 bytes)
  #@changeAST 1.2*2.1*a[1]*2.04*b[2]*3.2*c[1]*d[1]*1.12*3.2,12 # 382.929 ns (8 allocations: 640 bytes)
  #@changeAST 1.2*2.1*a[1]*2.04*b[2]*3.2*c[1]*d[1]*1.12*3.2*d[2],12 #436.455 ns (9 allocations: 720 bytes)
end
#@btime benchmarkboundaryMul(a,b,c,d,cache)
function stressTestMul(a::Vector{Taylor1{Float64}},b::Vector{Taylor1{Float64}},c::Vector{Taylor1{Float64}},d::Vector{Taylor1{Float64}},cache::Vector{Taylor1{Float64}})
  #test 2 vars
        ...
        for term3 in(2.07,c[1]) 
          if term1==1.2 && term2==1.33 && term3==2.07
           ...
           # @show cache
            ...
            clearCache(cache)
              @assert term1*term2*term3==@changeAST term1*term2*term3,12
          end
        end
      end
  end =#
        ...
        for term3 in(2.07,c[1]) 
          for term4 in(1.07,d[1]) 
              ...
              # @show cache
                ...
                clearCache(cache)
                  @assert term1*term2*term3*term4==@changeAST term1*term2*term3*term4,12
              end
           end
        end
      end
            ...
            for term4 in(1.07,d[1]) 
              for term5 in(0.7,a[2])
                  ...
                  # @show cache
                    ...
                    clearCache(cache)
                      @assert term1*term2*term3*term4*term5==@changeAST term1*term2*term3*term4*term5,12
                  end
                end
             end
          end
