      ...
      absDeltaT=1e-12# max is e-16 !!!!!!!!
        if (x[i].coeffs[3]) != 0
          #println("quantum[$i] = ",quantum[i])
            ...
            tempStep=max(sqrt(abs(quantum[i] / ((x[i].coeffs[3])*2))),absDeltaT)
            if tempStep!=absDeltaT #normal
                nextTime[i] = currentTime + tempStep#sqrt(abs(quantum[i] / ((x[i].coeffs[3])*2))) #*2 cuz coeff contains fact()
             ...
             # println("(x[$i].coeffs[3])*2 = ",(x[i].coeffs[3])*2)
              ...
              nextTime[i] = currentTime + tempStep
