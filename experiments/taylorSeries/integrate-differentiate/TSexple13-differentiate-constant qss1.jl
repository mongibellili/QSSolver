   ...
   function qss_Integrate(initCond::Vector{Float64},f::SVector{N, Function},order::Int)where{N}
      ...
      #  initTime= 1e-152 # only a not too small number works
        ...
        for i = 1:states
            ...
            computeNextInputTime(Val(1), i, initTime,0.0,0.0+t,f[i], nextInputTime, x,q, quantum)
        end
           ...
           # x[index].coeffs[2]=((differentiate(x[index]))(t+elapsed)).coeffs[1]
            ...
            if sch[3] ==:ST_STATE
                        computeNextTime(Val(1), index, simt, nextStateTime, x, quantum) #
                          ...
                          if Debug println("loop $count : nextStateTime after computenexttime= ",nextStateTime) end
                                ...
                                if j != 0   
                                  ...
                                  computeDerivative(Val(1),j, f[j], x, q, simt,t)
                       ...
                       # computeDerivative(Val(2),index, f[index], x, q, simt+t)
                        computeNextInputTime(Val(1), index, simt,elapsed,t,f[index], nextInputTime, x,q, quantum)
                          if Debug println("under inputchange loop $count : nextInputTime after computenextInputtime= ",nextInputTime) end
                          computeDerivative(Val(1),index, f[index], x, q, simt,t)
...
#=           else#input 
  ...
  function computeNextInputTime(::Val{1}, i::Int, currentTime::Float64,elapsed::Float64, t::Taylor1{Float64} ,f::Function,nextInputTime::MVector{T,Float64}, x::Vector{Taylor1{Float64}},q::Vector{Taylor1{Float64}}, quantum::MVector{T,Float64})where{T}
    df=0.0
    tt=Taylor1([0.0,0.0],2)
    oldDerX=((x[i].coeffs[2]))
    newDerX=(tt+f(q,currentTime+t)).coeffs[1] # do not put /factorial(2) cuz here we actually need derder not store the coeff
    println("-------------------------------------------",f(q,t))
      if elapsed > 0
        println("elapsed= ",elapsed) 
        println("oldDerDerX= ",oldDerX) 
        println("newDerDerX= ",newDerX) 
  ...
  function computeDerivative( ::Val{1} ,j::Int, f::Function ,x::Vector{Taylor1{Float64}} , q::Vector{Taylor1{Float64}},simt::Float64, t::Taylor1{Float64}  )
