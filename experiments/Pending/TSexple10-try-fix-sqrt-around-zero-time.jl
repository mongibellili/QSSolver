...
module modqss
       ...
       dep= @SVector[[1]]
        ...
        if Debug println("initialsavedVars  = ",savedVars) end
         for k=1:order # compute initial derivatives for x and q
          ...
          # println("k $k q= ",q) 
           for i=1:states 
            x[i].coeffs[k+1] =((differentiate(f[i](q,initTime),k-1)).coeffs[1] )/factorial(k) # /fact cuz i will store der/fac like the convention...to extract the derivatives (at endof sim) multiply by fac  derderx=coef[3]*fac(2)
           end
        end     
        if Debug       
 ...
 #---------------------------------------------------------------------------------while loop-------------------------------------------------------------------------
            ...
            for i = 1:length(dep[index])
                ...
                if j != 0   
                 # println("dependency loop $j= ")              
                  ...
                  computeDerivative(Val(2),j, f[j], x, q, simt)
    ...
    function computeDerivative( ::Val{2} ,j::Int, f::Function ,x::Vector{Taylor1{Float64}} , q::Vector{Taylor1{Float64}}, t::Float64  )
...
order=2
Debug=false
function f1(q::Vector{Taylor1{Float64}},t::Float64)
