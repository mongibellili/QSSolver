...
function linear(coeff::Vector{Float64})
  ...
  function quadratic(coeff::Vector{Float64})
    println("quadratic called")
    if coeff[3] == 0.0
      res = linear(coeff[1:2])
    else
      res = Array{Complex{Float64}}(undef,2)
      if coeff[1] == 0.0
        res[1] = 0.0
  ...
  function cubic(coeff::Vector{Float64})
    if coeff[4] == 0.0
      ...
      res = Array{Complex{Float64}}(undef,3)
      if coeff[1] == 0.0
        res[1] = 0.0
  ...
  function quartic(coeff::Vector{Float64})
    if coeff[5] == 0.0
      ...
      res = Array{Complex{Float64}}(undef,4)
      if coeff[1] == 0.0
        res[1] = 0.0
  ...
  function roots(coeff::Vector{Float64}) :: Vector{Complex{Float64}}
    ...
    elseif n == 5
      ...
      if coeff[n] == 0.0
        ...
        res = Array{Complex{Float64}}(undef,n-1)
        if coeff[1] == 0.0
          res[1] = 0.0
          res[2:n-1] = roots(coeff[2:n])
        else
          mat = zeros(Float64, n-1, n-1)
          #mat[2:n-1, 1:n-2] = eye(Float64, n-2) #eye deprecated
          mat[2:n-1, 1:n-2] = I(n-2)
...
p1=[-2.0,1.0,1.0]
p2=[-2.0,1.0,2.0,1.5,-2.3,1.0]
a = roots(p2)
r = filter(v->abs(imag(v)) < 1.0e-15 && real(v)>=0.0, [a...])
println(r)