...
lows = pointer(Vector{Float64}(undef, 5))
highs = pointer(Vector{Float64}(undef, 5))
@btime smallestpositiverootintervalnewtonregulafalsi((-1.33,2.0,1.0,6.0),lows,highs)
	...
	for n in 3:3
		...
		open("polynomials$n.txt", "r") do io
			@showprogress for i = 1:N
				...
				times[i] = 100000 * @belapsed allrealrootintervalnewtonregulafalsi($p, $res, $lows, $highs) samples=100 evals=10
			end
		end
	...
	for n in 3:3
		...
		open("polynomials$n.txt", "r") do io
			@showprogress for i = 1:N
				str = readline(io)
				p = tuple(parse.(Float64, split(str))...)
            	times[i] = 100000 * @belapsed smallestpositiverootintervalnewtonregulafalsi($p, $lows, $highs) samples=100 evals=10
			end
        end
		show(histogram(times))
		println()
