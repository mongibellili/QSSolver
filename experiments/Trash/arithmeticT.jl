...
#only one cache is sufficient: the reason i had two caches is that before i was reusing cache 1
#but since in the new method each op has its own cache, cache1 will be empty and different from a
...
function muladd(a::P,b::Q,c::R,cache1::Taylor0{T}) where {P,Q,R <:Union{Taylor0,Number},T<:Number}
...
function clearCache(cache::Vector{Taylor0{T}}) where {T<:Number}
