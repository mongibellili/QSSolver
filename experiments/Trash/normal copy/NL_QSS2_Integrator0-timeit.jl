...
#=     if Debug
  ...
  #=    for k = 1:order #evaluate x at new time, and derivatives...remember coeffs store the over facto also              
   ...
   @timeit "in state integratestate" integrateState(Val(2),x[index],cacheT,elapsed)
...
#=     if Debug
    ...
    tx[index] = simt
    @timeit "forloop q update" for k = 1:order
      ...
      for i = 1:length(SD[index])
       ...
       #    println("dependency loop $j= ")              
          ...
          if elapsed > 0
            ...
            @timeit "small q update"  q[j].coeffs[1] = x[j].coeffs[1]
            tx[j] = simt
            tq[j] = simt
        ...
        #  @timeit "in state fbang" fbang[j](x,q,d,t)
       #  @timeit "in state integrate!"  integrate!(x[j],cacheT,x[j].coeffs[1])
