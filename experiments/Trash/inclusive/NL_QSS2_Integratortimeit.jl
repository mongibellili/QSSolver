...
#=     if Debug
   ...
   #=    if allowPrint>0
     ...
     # println("at time $simt state change  index= ",index)
     @timeit "state:comnexttime" computeNextTime(Val(2), index, simt, nextStateTime, x, quantum) #
      for i = 1:length(SD[index])
       ...
       #    println("dependency loop $j= ")              
        ...
        # @timeit "state:comDer1"  computeDerivative1(Val(2), j,x, f[j](q,d,t),cacheT,simt)
         @timeit "state:comDer1" computeDerivative1(Val(2), j,tq,x,q,d,t, f[j],cacheT,simt)
         @timeit "state:comDer2"   computeDerivative2(Val(2), j,x, f[j](q,d,t),cacheT,simt)
         @timeit "state:recomnexttime" reComputeNextTime(Val(2), j, simt, nextStateTime, x, q, quantum)
        #=  if allowPrint>0
      ...
      for i = 1:length(SZ[index])
        j = SZ[index][i] # this line before the next line or vice versa gave the same bench results
         if j != 0             
          ...
          @timeit "state:recomnextevent" computeNextEventTime(j,zcf[j](x,d,simt + t),oldsignValue,simt,  nextEventTime, quantum)
